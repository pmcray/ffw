    ...
    for side in (IMPERIAL, ZHODANI):
        ...
        for g in range(games):
            ...
            for on in (False, True):
                ...
                play(state,
                     ScriptedAgent(IMPERIAL, seed=seed,
                                   pickets=on and side == IMPERIAL),
                     ScriptedAgent(ZHODANI, seed=seed + 1,
                                   pickets=on and side == ZHODANI),
                     max_turns=max_turns)
                outcome[on] = state.victory_margin()
            change = outcome[True] - outcome[False]
            gains.append(change if side == ZHODANI else -change)
        mean = sum(gains) / len(gains)
        var = sum((a - mean) ** 2 for a in gains) / max(1, len(gains) - 1)
        stderr = (var / len(gains)) ** 0.5
        ...
        print("== %-8s pickets on vs off: %+7.1f +/- %5.1f VP over %d paired"
              " games -> %s ==   %.0fs"
              % (side, mean, stderr, games, verdict, time.time() - start),
              flush=True)
